...
secretome.data <- pamr.from.excel("LFQintensity_no_zeros_input.txt", 40, sample.labels=TRUE)
...
for (rep in 1:repetition){
  ...
  ref=list()
  for (i in 1:nfold){
    cat("definindo conjunto de treinamento outer-loop:", i,"\n")
    ...
    secretome2.testY <- secretome.data$y[folds[[i]]]
        ...
        for (ii in 1:nfold_inner){
          cat("definindo conjunto de treinamento innet-loop:",ii,"\n")      
          ...
          cat("\ntreinamento usando scales-inner\n")
          secretome2_inner.train2 <- pamr.train(secretome2_inner.data, n.threshold=thresholds_number) 
          
          #teste
          cat("\nteste de um modelo inner\n")
    ...
    cat("\nvalores de thresholds selecionados no inner\n")
...
#      
    ...
    cat("\nsalvando modelo para loop do double\n")
...
cat("\nselecionando maior valor de threshold\n")
  for (tr in 1:nfold){
  ...
  cat("\nselecionando modelo de menor erro e max probabilidade\n")
  #treinamento final
  ...
  cat("\nreexecutando treinamento com o conjunto completo para gerar o ranking final\n")
  secretome.train2 <- pamr.train(secretome.data, threshold.scale=secretome.scales)  
  cat("\ncortando a lista pelo threshold especificado\n")
...
write.csv(final_genes,"final_gene_list.csv")

#png(filename="gene_plot_NSC.png", width=4000, height=4000, pointsize=1/500)
png(filename="gene_plot_NSC.png", width=4000, height=4000, pointsize=12)
