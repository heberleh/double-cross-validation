...
svmrfeFeatureRanking = function(x,y){
    ...
    while(length(survivingFeaturesIndexes)>0){
        ...
        svmModel = svm(x[, survivingFeaturesIndexes], y, cost = 10, cachesize=500,  scale=T, type="C-classification", kernel="linear" )
...
#nonzeros <- colSums(abs(secretome.x)>0)
#remove cols with only zeros
#secretome.x <- secretome.x[,nonzeros]
...
#for(i in 1:ncol(secretome.x))secretome.x[,i] = (secretome.x[,i]-mean(secretome.x[,i]))/sd(secretome.x[,i])
#x = 2*atan(x/2)

# remove o conjunto de teste após normalizar
test_index = c(3,11,13)
secretome.x <- secretome.x[-test_index,]
secretome.y <- secretome.y[-test_index]
...
#for (rep in 1:repetition){
  ...
  for (i in 1:nfold){
    ...
    for (ii in 1:nfold_inner){
      ...
      # Não pode ser executado aqui, devido haver apenas 15 amostras 5-fold, seguido de 3-fold. 
      # a função Tune não consegue fazer mais um level de folds.
...
write.matrix(featureRankedList, file ="rank_index_svmrfe.csv", sep=",")


write(min_N, file="double_selected_N_svmrfe.txt")
